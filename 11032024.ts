// Calculate the sum of binary tree nodes between two values (optimized).
...
function rangeSumBST(root: TreeNode | null, low: number, high: number): number {
  ...
  // Recursive solution.
  (function DFS(node: TreeNode | null): null {
    if (!node) {
      return null;
    }
    ...
    // Move only to the left and right nodes if the current node is within the respective bound.
    if (val > low) {
      DFS(left);
    }
